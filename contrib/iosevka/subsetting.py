...
The script expects a collection of Iosevka font files with a complete set of character sets.
...
Then run the script with the desired mode using the command `uv run subsetting.py`.
The complete list of command line options is shown below:

```bash
$ uv run subsetting.py --help          # view commands
$ uv run subsetting.py --dry-run       # dry run (no file generation), default
$ uv run subsetting.py --css-only      # generate CSS files only
$ uv run subsetting.py --subsets-only  # generate WOFF2 files only
$ uv run subsetting.py --all           # generate both CSS and WOFF2 files
```
"""
...
class IosevkaFontInfo:
    ...
    weights_map: t.ClassVar[list[tuple[str, Weight]]] = [
        ...
        ("Light", 300),
        ("", 400),
        ("Regular", 400),  # empty value preferred
        ("Medium", 500),
        ("SemiBold", 600),
        ("Bold", 700),
        ("ExtraBold", 800),
